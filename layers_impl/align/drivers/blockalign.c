...
int write_block(char *buf, size_t size, uint64_t offset, struct io_info *inf) {
    ...
    // DEBUG_MSG("Going to write to next layer: size=%lld, offset=%lld\n", size, offset);
...
int process_write_block(char *buf, size_t size, uint64_t block_offset, uint64_t block_extra_offset,
    ...
    if (size == BLOCKSIZE && block_extra_offset == 0) {
        // DEBUG_MSG("size == BLOCKSIZE && block_extra_offset == 0");
        write_buf = buf;
        bytes_to_write = BLOCKSIZE;

    } else {
        if (file_size == 0) {
            // DEBUG_MSG("file_size = 0\n");
            // Get the file size
            //TODO: this can be optimized with a cache
        ...
        if (block_offset >= file_size) {
            ...
            // DEBUG_MSG("block_offset < file_size\n");
            ...
            if (block_extra_offset == 0 && remaining_file_bytes <= size) {
                ...
                // DEBUG_MSG("Going to read from next layer: size=%lld, block_offset=%lld\n", bytes_to_read, block_offset);
...
int split_into_blocks(int io_type, char *buf, size_t size, uint64_t aligned_offset, uint64_t block_extra_offset,
                      struct io_info *inf) {
    // DEBUG_MSG("Entering split_into_block: size=%lld, aligned_offset=%lld, block_extra_offset=%lld\n", size, aligned_offset, block_extra_offset);
    ...
    while (buffer_bytes_processed < size) {
        ...
        // DEBUG_MSG(
        //     "split_into_blocks: Processing file Path %s, and block offset %llu, extra_offset %llu, size %zu, size_to_process %llu, buf "
        //     "pointer %zu\n",
        ...
        // DEBUG_MSG(
        //     "split_into_blocks: Processed file Path %s, and block offset %llu, extra_offset %llu, size %zu, size_to_process %llu, buf "
        //     "pointer %zu\n",
        //     inf->path, (unsigned long long int)next_offset_to_process, (unsigned long long int)next_block_extra_offset,
        //     size, (unsigned long long int)remaining_size_to_process, buffer_bytes_processed);
...
int block_align_read(const char *path, char *buf, size_t size, off_t offset, void *fi,
    ...
    // DEBUG_MSG("block_align_read: Path %s,  offset %llu, size %zu\n", path, (unsigned long long int)offset, size);
...
int block_align_write(const char *path, const char *buf, size_t size, off_t offset, void *fi,
    ...
    // DEBUG_MSG("block_align_write: Path %s,  offset %llu, size %zu\n", path, (unsigned long long int)offset, size);
